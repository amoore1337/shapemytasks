...
export default function HillChartSection({ isMobile }: { isMobile: boolean }) {
  return isMobile ? (
    ...
    <div className="flex justify-between items-center px-10 w-full mt-20" style={{ maxWidth: MAX_WIDTH }}>
      <Callout />
      <Visual />
    </div>
  );
...
function Callout({ isMobile }: { isMobile?: boolean }) {
  return (
    <div>
      <Typography variant={isMobile ? 'h4' : 'h3'} component="h2" className="relative" maxWidth={isMobile ? 360 : MAX_TITLE_WIDTH}>
...
function Visual({ isMobile }: { isMobile?: boolean }) {
  return (
    <div className={`flex items-center justify-center ${isMobile ? '' : 'mt-8'}`} style={{ height: isMobile ? 175 : 300, width: isMobile ? 375 : 650 }}>
      <HillChart
        width="100%"
        height="100%"
        data={scopes}
