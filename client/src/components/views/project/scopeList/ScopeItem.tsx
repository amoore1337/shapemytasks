...
export default function ScopeItem({
  ...
  return (
    <div ref={(node) => dropRef(preview(node))} className="flex justify-between p-2">
      <div className="flex flex-grow items-center">
        ...
        >
          <DragIcon className={dragEnabled ? 'cursor-move text-gray-400' : 'text-gray-100'} />
        </button>
        ...
        {isFlagged && (
              ...
              <div className="text-xs">
                {scope.flag!.message ? (
                  <>
                    ...
                    <div className="mt-1 text-right">- {flagger}</div>
                  </>
                ) : (
                  <div className="italic">Flagged by {flagger}</div>
                )}
              </div>
            }
            placement="top"
          ...
          >
            <FlagIcon className="ml-2 text-danger" />
          </Tooltip>
        )}
        <Typography
          className={`ml-2 ${inProgress ? 'font-bold' : ''} ${completed ? 'line-through' : ''}`}
          style={{ maxWidth: '70%' }}
        >
      ...
      />
      <EditScopeModal scope={scope} open={showEditModal} onClose={handleCancelAction} />
      <AddFlagModal scope={scope} open={showFlagModal} onClose={handleCancelAction} />
